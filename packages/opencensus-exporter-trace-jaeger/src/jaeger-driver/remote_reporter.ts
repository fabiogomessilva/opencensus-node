...
import {LogData, Process, Reference, Tag} from './jaeger-thrift';
import {Sender} from './reporter';
import {types, logger}  from '@opencensus/opencensus-core';
  ...
  _appendCallback = (numSpans: number, err?: string) => {
    if (err) {
     ...
     // this._metrics.reporterDropped.increment(numSpans);
      ...
      this._logger.debug(`Sucefull append`);
